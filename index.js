...
const parseImports = (options, blob) => {
  const split = blob.source.split(';')
  const imports = []
  split.forEach(line => {
    if (line.indexOf('import') > -1) {
      const path = line.split('import')[1].replace(/['"]+/g, '').replace(' ', '')
      const split = path.split('/')
      const name = split[split.length - 1].replace('.sol', '')
...
const getAllArtifacts = async (options) => {
  const files = fs.readdirSync(options.contractsBuildDir)
  const content = []
  files.forEach(file => {
